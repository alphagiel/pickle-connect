...
interface DoublesMatchResultEmailData {
...
export function doublesMatchResultEmail(data: DoublesMatchResultEmailData): string {
  const resultText = data.isWinner ? "Your team won!" : "Better luck next time!";
...
<h2 style="margin: 0 0 16px 0; color: #333333; font-size: 20px; font-weight: 600;">
  Doubles Match Result Recorded
</h2>

<p style="margin: 0 0 16px 0; color: #555555; font-size: 16px; line-height: 1.6;">
  Hey ${data.recipientName}, the results for your doubles match have been recorded.
</p>
  ...
  <tr>
    <td style="padding: 20px;">
      <p style="margin: 0 0 12px 0; color: #333333; font-size: 16px;">
        <strong>Your Team:</strong> ${data.recipientName} & ${data.partnerName}
      </p>
      <p style="margin: 0 0 16px 0; color: #333333; font-size: 16px;">
        <strong>Opponents:</strong> ${data.opponent1Name} & ${data.opponent2Name}
      </p>
      <p style="margin: 0 0 12px 0; color: #333333; font-size: 16px;">
...
export function doublesMatchResultEmailSubject(isWinner: boolean): string {
  return isWinner
    ? "Your team won the doubles match!"
    : "Doubles match result recorded";
}
